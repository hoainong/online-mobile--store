...
const ProductCard = ({product}) => {
  ...
  return (
    <Link to={`/product-detail/${product.id}`} className='productCard relative w-[20rem] m-3 transition-all cursor-pointer border-2 rounded-md'>
      <div className='h-[20rem]'>
        <img className='h-full w-full object-contain object-left-top rounded-md pt-3'
      ...
      <div className='textPart bg-white p-3'>
        <div>
          ...
          <p className='p-2 inline-block rounded border border-black font-mono'>{product.rom}</p>
        </div>
